...
class Product:
    """A store product, including its tax behaviour."""
    def __init__(self, name, price, category):
        ...
        self.name, self.price, self.category = name, float(price), category

    def tax_for(self, line_total):
        rate = FOOD_TAX_RATE if self.category == FOOD_CATEGORY else TAX_RATE
        return line_total * rate
...
class OrderItem:
    ...
    def __init__(self, product, quantity):
        if not isinstance(product, Product):
            raise TypeError("OrderItem requires a Product.")
...
def make_customer(name, tier):
    ...
    try:
        return customer_types[tier](name)
    except KeyError:
        raise ValueError("Unknown membership tier: " + str(tier))


class Order:
    """An order has one customer and multiple order items."""
    def __init__(self, customer, items):
        if not isinstance(customer, Customer):
            raise TypeError("Order requires a Customer.")
        self.customer = customer
        self.items = list(items)
        if any(not isinstance(item, OrderItem) for item in self.items):
            raise TypeError("Every order entry must be an OrderItem.")
    ...
    def discount(self):
        ...
        member_discount = subtotal * self.customer.discount_rate(subtotal)
        quantity = sum(item.quantity for item in self.items)
        bulk_discount = subtotal * BULK_DISCOUNT_RATE if quantity >= BULK_QTY_THRESHOLD else 0.0
        return member_discount + bulk_discount

    def tax(self):
        return sum(item.product.tax_for(item.line_total()) for item in self.items)
    ...
    def receipt(self):
        """Return the receipt string; do not print or calculate during display."""
        lines = [
...
def build_orders():
    ...
    result = []
    for name, tier, entries in ORDERS:
        customer = make_customer(name, tier)
        items = [OrderItem(products[index], quantity) for index, quantity in entries]
